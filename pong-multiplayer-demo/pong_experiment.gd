...
#func _ready():
	...
	## set_multiplayer_authority is tree-recursive by default.
		...
		#player2.set_multiplayer_authority(multiplayer.get_peers()[0])#YOU CAN JUST USE THIS!
	#else:
...
func _ready():
	...
	if multiplayer.is_server():
		# For the server, give control of player 2 to the other peer.
		player_2.set_multiplayer_authority(multiplayer.get_peers()[0])#YOU CAN JUST USE THIS!
	else:
		# For the client, give control of player 2 to itself.
		player_2.set_multiplayer_authority(multiplayer.get_unique_id())
	...
	#
		...
		#return
		#
	#multiplayer.peer_connected.connect(assign_player)
	...
	#multiplayer.peer_connected.connect(assign_player) #THIS IS FOR WEBSOCKET
